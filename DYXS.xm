...
//  DYXS
//
//  Copyright (c) 2024 xiaoshuai. All rights reserved.
//  Channel: @cxs-dev
...
- (void)setIsAutoPlay:(BOOL)arg0 {
    float defaultSpeed = [[NSUserDefaults standardUserDefaults] floatForKey:@"DYXSDefaultSpeed"];
...
+ (id)button {
    BOOL isHiddenJia = [[NSUserDefaults standardUserDefaults] boolForKey:@"DYXSisHiddenJia"];
    if (isHiddenJia) {
        return nil;
    }
    return %orig;
...
- (void)setAlpha:(CGFloat)alpha {
    NSString *transparentValue = [[NSUserDefaults standardUserDefaults] objectForKey:@"DYXStopbartransparent"];
    
    if ([[NSUserDefaults standardUserDefaults] boolForKey:@"DYXSisEnablePure"]) {
        %orig(0.0);
...
- (void)setTextColor:(UIColor *)textColor {
    if ([[NSUserDefaults standardUserDefaults] boolForKey:@"DYXSEnableDanmuColor"]) {
        NSString *danmuColor = [[NSUserDefaults standardUserDefaults] objectForKey:@"DYXSdanmuColor"];
...
//            UIViewController *settingVC = [[NSClassFromString(@"DYXSSettingViewController") alloc] init];
//            if (settingVC) {
...
- (void)setHidden:(BOOL)hidden {
    if ([[NSUserDefaults standardUserDefaults] boolForKey:@"DYXSHideAvatarButton"]) {
        hidden = YES;
    }
...
- (void)setHidden:(BOOL)hidden {
    if ([[NSUserDefaults standardUserDefaults] boolForKey:@"DYXSisHiddenEntry"]) {
        hidden = YES;
    }
...
- (void)live_callInitWithDictyCategoryMethod:(id)arg1 {
    if (self.currentAweme && [self.currentAweme isLive] && [[NSUserDefaults standardUserDefaults] boolForKey:@"DYXSisSkipLive"]) {
        return;
    }
    %orig;
}
...
+ (id)liveStreamURLJSONTransformer {
    return [[NSUserDefaults standardUserDefaults] boolForKey:@"DYXSisSkipLive"] ? nil : %orig;
}
...
- (void)viewDidLayoutSubviews {
    ...
    if ([[NSUserDefaults standardUserDefaults] boolForKey:@"DYXSisEnableFullScreen"]) {
        CGRect frame = self.view.frame;
        frame.size.height = self.view.superview.frame.size.height - 83;
        self.view.frame = frame;
    }
    
    BOOL shouldHideSubview = [[NSUserDefaults standardUserDefaults] boolForKey:@"DYXSisEnableFullScreen"] || 
                             [[NSUserDefaults standardUserDefaults] boolForKey:@"DYXSisEnableCommentBlur"];
...
- (void)setFrame:(CGRect)frame {
    if (![[NSUserDefaults standardUserDefaults] boolForKey:@"DYXSisEnableCommentBlur"]) {
        %orig;
        return;
    }
...
- (void)setAlpha:(CGFloat)alpha {
    ...
    if ([vc isKindOfClass:%c(AWEPlayInteractionViewController)] && alpha > 0) {
        NSString *transparentValue = [[NSUserDefaults standardUserDefaults] stringForKey:@"DYXSGlobalTransparency"];
...
- (void)layoutSubviews {
    %orig;
    if ([[NSUserDefaults standardUserDefaults] boolForKey:@"DYXSisHiddenBottomDot"]) {
        for (UIView *subview in [self subviews]) {
...
- (void)layoutSubviews {
    %orig;
    if ([[NSUserDefaults standardUserDefaults] boolForKey:@"DYXSisHiddenSidebarDot"]) {
        for (UIView *subview in [self subviews]) {
            if ([subview isKindOfClass:NSClassFromString(@"DUXBadge")]) {
                subview.hidden = YES;
            }
...
- (void)layoutSubviews {
    ...
    if ([accessibilityLabel isEqualToString:@"点赞"]) {
        if ([[NSUserDefaults standardUserDefaults] boolForKey:@"DYXSHideLikeButton"]) {
            [self removeFromSuperview];
            return;
        }
    } else if ([accessibilityLabel isEqualToString:@"评论"]) {
        if ([[NSUserDefaults standardUserDefaults] boolForKey:@"DYXSHideCommentButton"]) {
            [self removeFromSuperview];
            return;
        }
    } else if ([accessibilityLabel isEqualToString:@"分享"]) {
        if ([[NSUserDefaults standardUserDefaults] boolForKey:@"DYXSHideShareButton"]) {
            [self removeFromSuperview];
            return;
        }
    } else if ([accessibilityLabel isEqualToString:@"收藏"]) {
        if ([[NSUserDefaults standardUserDefaults] boolForKey:@"DYXSHideCollectButton"]) {
            [self removeFromSuperview];
            return;
        }
    }
...
- (void)layoutSubviews {
    ...
    if ([accessibilityLabel isEqualToString:@"音乐详情"]) {
        if ([[NSUserDefaults standardUserDefaults] boolForKey:@"DYXSHideMusicButton"]) {
            [self removeFromSuperview];
            return;
        }
    }
...
- (void)layoutSubviews {
    ...
    BOOL hideShop = [[NSUserDefaults standardUserDefaults] boolForKey:@"DYXSHideShopButton"];
    BOOL hideMsg = [[NSUserDefaults standardUserDefaults] boolForKey:@"DYXSHideMessageButton"];
    BOOL hideFri = [[NSUserDefaults standardUserDefaults] boolForKey:@"DYXSHideFriendsButton"];
    ...
    if ([[NSUserDefaults standardUserDefaults] boolForKey:@"DYXSisHiddenBottomBg"] || [[NSUserDefaults standardUserDefaults] boolForKey:@"DYXSisEnableFullScreen"]) {
        for (UIView *subview in self.subviews) {
            if ([subview class] == [UIView class]) {
...
- (void)setKeyboardAppearance:(UIKeyboardAppearance)appearance {
    if ([[NSUserDefaults standardUserDefaults] boolForKey:@"DYXSisDarkKeyBoard"]) {
...
//    NSLog(@"拦截到关注按钮点击");
    if ([[NSUserDefaults standardUserDefaults] boolForKey:@"DYXSfollowTips"]) {
...
- (id)touchUpInsideBlock {
    ...
    if ([[NSUserDefaults standardUserDefaults] boolForKey:@"DYXScollectTips"] && [self.accessibilityLabel isEqualToString:@"收藏"]) {
        dispatch_async(dispatch_get_main_queue(), ^{
            UIAlertController *alertController = [UIAlertController
...
- (void)setAlpha:(CGFloat)alpha {
    if ([[NSUserDefaults standardUserDefaults] boolForKey:@"DYXSisShowSchedule"]) {
        alpha = 1.0;
        %orig(alpha);
    }else {
        %orig;
...
- (void)layoutSubviews {
    ...
    NSString *indexTitle = [[NSUserDefaults standardUserDefaults] objectForKey:@"DYXSIndexTitle"];
    NSString *friendsTitle = [[NSUserDefaults standardUserDefaults] objectForKey:@"DYXSFriendsTitle"];
    NSString *msgTitle = [[NSUserDefaults standardUserDefaults] objectForKey:@"DYXSMsgTitle"];
    NSString *selfTitle = [[NSUserDefaults standardUserDefaults] objectForKey:@"DYXSSelfTitle"];
...
- (BOOL)isAutoPlayOpen {
    ...
    if ([[NSUserDefaults standardUserDefaults] boolForKey:@"DYXSisEnableAutoPlay"]) {
        return YES;
    }
    return r;
}
...
- (void)setChannelID:(NSString *)channelID {
    ...
    if (([channelID isEqualToString:@"homepage_hot_container"] && [defaults boolForKey:@"DYXSHideHotContainer"]) ||
        ([channelID isEqualToString:@"homepage_follow"] && [defaults boolForKey:@"DYXSHideFollow"]) ||
        ([channelID isEqualToString:@"homepage_mediumvideo"] && [defaults boolForKey:@"DYXSHideMediumVideo"]) ||
        ([channelID isEqualToString:@"homepage_mall"] && [defaults boolForKey:@"DYXSHideMall"]) ||
        ([channelID isEqualToString:@"homepage_nearby"] && [defaults boolForKey:@"DYXSHideNearby"]) ||
        ([channelID isEqualToString:@"homepage_groupon"] && [defaults boolForKey:@"DYXSHideGroupon"]) ||
        ([channelID isEqualToString:@"homepage_tablive"] && [defaults boolForKey:@"DYXSHideTabLive"]) ||
        ([channelID isEqualToString:@"homepage_pad_hot"] && [defaults boolForKey:@"DYXSHidePadHot"]) ||
        ([channelID isEqualToString:@"homepage_hangout"] && [defaults boolForKey:@"DYXSHideHangout"])) {
        return;
    }
    %orig;
}
...
- (id)timestampLabel {
    UILabel *label = %orig;
    if ([[NSUserDefaults standardUserDefaults] boolForKey:@"DYXSisEnableArea"]) {
...
+(BOOL)shouldActiveWithData:(id)arg1 context:(id)arg2{
	return [[NSUserDefaults standardUserDefaults] boolForKey:@"DYXSisEnableArea"];
}
...
- (BOOL)prefersStatusBarHidden {
    if([[NSUserDefaults standardUserDefaults] boolForKey:@"DYXSisHideStatusbar"]){
        return YES;
    } else {
        return %orig;
    }
...
//        NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
//        [defaults registerDefaults:@{@"isShowDYXSAlert": @(NO)}];
//
//        if (![defaults boolForKey:@"isShowDYXSAlert"]) {
//            [defaults setBool:YES forKey:@"isShowDYXSAlert"];
//            [defaults synchronize];
//
//            UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"by @cxs-dev"
...
//                    NSURL *tgURL = [NSURL URLWithString:@"tg://resolve?domain=cxs-dev"];
...
- (void)setAlpha:(CGFloat)alpha {
    if ([[NSUserDefaults standardUserDefaults] boolForKey:@"DYXSHideDiscover"]) {
        alpha = 0;
        %orig(alpha);
   }else {
       %orig;
...
- (void)layoutSubviews {
    ...
    if ([[NSUserDefaults standardUserDefaults] boolForKey:@"DYXSHideMyPage"]) {
        [self removeFromSuperview];
        return;
    }
}
...
- (void)layoutSubviews {
    ...
    if ([[NSUserDefaults standardUserDefaults] boolForKey:@"DYXSHideLocation"]) {
        [self removeFromSuperview];
        return;
    }
}
...
- (void)layoutSubviews {
    ...
    if ([[NSUserDefaults standardUserDefaults] boolForKey:@"DYXSHideInteractionSearch"]) {
        [self removeFromSuperview];
        return;
    }
}
...
- (void)layoutSubviews {
    ...
    if ([[NSUserDefaults standardUserDefaults] boolForKey:@"DYXSHideQuqishuiting"]) {
        self.hidden = YES;
    }
}
...
- (void)layoutSubviews {
    ...
    if ([[NSUserDefaults standardUserDefaults] boolForKey:@"DYXSHideHotspot"]) {
        [self removeFromSuperview];
        return;
    }
}
